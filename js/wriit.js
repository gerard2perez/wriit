...
var installedplugins = ['bold', /*'pasteEvent',  'italic', 'underline', 'strikethrough', 'pown', 'subindex'*/ , 'undo', 'redo', 'paragraph'];
...
function NodeAnalysis(tags, maincontainer) {
	...
	if (leftNode == rightNode) {
		common = leftNode;
		findAllTags(common, middle, tags);
	} else {
	...
	for (let prop in tags) {
		let tag = tags[prop];
		if(contain[tag.Id])plugs.isSourrounding[tag.Id] = contain[tag.Id] ? contain[tag.Id]:null;
		if(contain[tag.Id])plugs.isSourrounding[tag.Id] = contain[tag.Id] ? contain[tag.Id]:null;
		if(contain[tag.Id])plugs.isSourrounding[tag.Id] = contain[tag.Id] ? contain[tag.Id]:null;
		if(contain[tag.Id])plugs.isSourrounding[tag.Id] = contain[tag.Id] ? contain[tag.Id]:null;
		plugs[tag.Id] = {
...
function FIND(text, parentag, tag) {
	let subtags = MatchNT(text, tag.FullMatch);
	let others = MatchNT(text, parentag.MatchTag);
	let fob = findNT(text, tag.OpenTag) - subtags;
	let fcb = findNT(text, tag.CloseTag) - others;
	return {
		open: fob,
		close: fcb
	};
...
function Wriit(parent, cfg) {
	...
	this.textarea.bind('keyup mouseup', function () {
		that.plugins = NodeAnalysis.apply(that.textarea.getSelection(0).rang, [that.tags, that.textarea.get(0)]);
	});
	...
	this.cfg.plugins.forEach(function (plugin) {
		prototype[plugin] = $.extend({
			...
			AddButton: function (button, classicon, tooltip) {
				let fns = that.textarea.data('fns') || {};

				function add(button) {
					let newB = $('<button data-wriit-commandId="' + button.Id + '" class="' + button.DisplayClass + '"><span>' + button.ToolTip + '</span></button>');
					that.buttons["_new_" + button.Id] = button;
					that.tags[button.Id] = button;
					that.menu.append(newB);
					return button.Id;
				}
				if (button instanceof Basic) {
					this.tagid = add(button);
				...
				fns[button.Id] = function () {
					tagAnalysisV4.apply(that, [compiled, button]);
				};
				that.textarea.bind('keyup mouseup', fns[button.Id]);
				that.textarea.data('fns', fns);
...
Wriit.prototype.bold = {
	setup: function () {
		let bold = new Basic("bold", "strong", {
			tooltip: "Bold",
